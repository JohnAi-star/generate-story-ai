...
function ViewStory({ params }: any) {
  ...
  const getStory = async () => {
    try {
      ...
      console.log("Fetched Story:", result);  // Debug log to inspect fetched data
  ...
  const handleDownloadStory = () => {
    ...
    // Check if story.output and story.output.chapters are available
    if (!story.output || !story.output.chapters || story.output.chapters.length === 0) {
      console.error("No story content available to download.");
      return;
    }

    // Construct story text for download
    const storyText = `
Title: ${story.output?.story_cover?.title || "Untitled"}

${story.output?.chapters?.length
      ? story.output.chapters.map((chapter: any, index: number) => (
          `Chapter ${index + 1}:\n${chapter.content}\n\n`
        )).join('')
      : "No chapters are available for this story."}
    `;

    console.log("Generated Story Text:", storyText);  // Log the generated text
